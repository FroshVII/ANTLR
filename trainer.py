"""SNN training procedure from Kim et al, 2020."""
...
class Trainer:
    ...
    def __init__(
        self, config, data_loaders, logger, gpu=False, task="nmnist"
    ):
        """
    ...
    def make_model(self, model_config):
        ...
        Args
            ...
            same_init : A boolean variable that indicates wheter using
                same initial parameters for every models or not.
        """
    ...
    def make_scheduler(self):
        ...
        scheduler = torch.optim.lr_scheduler.StepLR(
            self.optim, self.config.lr_step_size, self.config.lr_gamma
        )
    ...
    def spikes2label(self, spmat, decision_type="count"):
        ...
        if decision_type == "count":
            ...
            decreasing_output = spmat * torch.arange(
                spmat.shape[1], 0, -1
            ).view(1, -1, 1)
    ...
    def float2spikes(
        ...
        if type == "stretch":
            flmat_min_to_max = (
                num_max_spikes - num_min_spikes
            ) * flmat + num_min_spikes
            # batch x features
            if skip_zero:
                flmat_min_to_max[flmat == 0] = 0
            flmat_min_to_max = flmat_min_to_max.view(batch_size, 1, -1)

            flmat_increasing_from_zero = flmat_min_to_max * torch.arange(
                0, 1 + 1 / time_length, 1 / time_length
            ).view(1, -1, 1)
            ...
            if skip_zero:
                flmat_min_to_max[flmat == 0] = 0
            flmat_min_to_max = flmat_min_to_max.view(batch_size, 1, -1)
            flmat_increasing_from_zero = (
                flmat_min_to_max - num_max_spikes
            ) + num_max_spikes * torch.arange(
                0, 1 + 1 / time_length, 1 / time_length
            ).view(
                1, -1, 1
            )
            flmat_increasing_from_zero = torch.clamp(
                flmat_increasing_from_zero, 0, num_max_spikes
            )
            ...
            type == "latency"
        ):  # num_max_spikes -> max_timing, num_min_spikes -> min_timing
            flmat_min_to_max = (
                num_max_spikes - num_min_spikes
            ) * flmat + num_min_spikes
            # batch x features
            ...
            flmat_min_to_max = flmat_min_to_max.view(batch_size, 1, -1)
            spmat = (
                torch.ones(flmat_min_to_max.shape)
                * torch.arange(0, time_length).view(1, -1, 1)
                == flmat_min_to_max.int()
            ).float()
    ...
    def run(self, config):
        ...
        if self.resume:
            if config.multi_model:
                best_valid_acc = np.array(
                    self.logger.valid_df["acc"].tolist()
                ).max(0)
                best_valid_acc_first = np.array(
                    self.logger.valid_df["acc_first"].tolist()
                ).max(0)
            else:
                best_valid_acc = self.logger.valid_df["acc"].to_numpy().max()
                best_valid_acc_first = (
                    self.logger.valid_df["acc_first"].to_numpy().max()
                )

            current_epoch = self.logger.valid_df["epoch"].to_numpy().max() + 1
        elif config.multi_model:
        ...
        for epoch in range(current_epoch, current_epoch + config.epoch):
            train_loss, train_acc, train_acc_first, kbar = self.run_epoch(
                "train", model, optim, scheduler, epoch
            )
            train_nums_total = (
                np.array(self.total_num_spike_total).sum()
                / self.train_num_data
            )
            train_nums_nec = (
                np.array(self.total_num_spike_nec).sum() / self.train_num_data
            )
            scheduler.step()
            self.logger.log_train_df(
                epoch,
                train_loss,
                train_acc,
                train_acc_first,
            ...
            valid_loss, valid_acc, valid_acc_first = self.run_epoch(
                "valid", model
            )
            kbar.add(
                0,
            ...
            valid_nums_nec = (
                np.array(self.total_num_spike_nec).sum() / self.valid_num_data
            )
            self.logger.log_valid_df(
                epoch,
                valid_loss,
                valid_acc,
                valid_acc_first,
            ...
            if config.multi_model:
                for m in (
                    (valid_acc.cpu().numpy() > best_valid_acc)
                    .nonzero()[0]
                    .tolist()
                ):
                ...
                for m in (
                    (valid_acc_first.cpu().numpy() > best_valid_acc_first)
                    .nonzero()[0]
                    .tolist()
                ):
            ...
            self.model.clean_state()  # reset model

    def run_epoch(self, mode, model, optim=None, scheduler=None, epoch=None):

        # Ensure a valid run mode is specified
        ...
        if model.multi_model and mode != "test":
        ...
        for batch_idx, inp_tar in enumerate(loader):
            ...
            # If training, run backward pass and update
            ...
            # If not training, calculate loss without updating weights
            ...
            # Bookkeep, then calculate and report summary statistics
            loss = model.loss
            ...
            if batch_idx == 0:
                ...
                self.total_num_spike_total = [
                    (
                        ...
                        + np.array(self.total_num_spike_total[i])
                    ).tolist()
                ...
                self.total_num_spike_nec = [
                    (
                        ...
                        + np.array(self.total_num_spike_nec[i])
                    ).tolist()
                ...
                self.min_first_stime_min = min(
                    self.min_first_stime_min, first_stime_min
                )
                self.mean_first_stime_mean = (
                    self.mean_first_stime_mean * progress
                    + first_stime_mean * batch_size
                ) / (progress + batch_size)
            ...
            if model.multi_model:
                num_correct = (
                    (pred_class.reshape(target.shape) == target)
                    .sum(1)
                    .float()
                )
                num_correct_first = (
                    (pred_class_first.reshape(target.shape) == target)
                    .sum(1)
                    .float()
                )
            ...
            # Update progress bar if training
            if mode == "train":
                kbar.update(
                    batch_idx + 1,
                    values=[
        ...
        if mode == "train":
            report = (*report, kbar)  # include progress bar data
        return report

    def test(self):
        test_loss, test_acc_most, test_acc_earliest = self.run_epoch(
            "test", self.model
        )
        print(
            f"test_loss : {test_loss:.4f} | test_acc_most : {test_acc_most:.4f} | "
    ...
    def visualize_activity(self, model, input, target):
        ...
        state_v = [
            torch.stack(v).reshape(model.term_length, batch_size, -1)
            if len(v) > 0
            else None
            for v in model.state_v
        ]
        len_state_v = [1 for sv in state_v if sv != None]
        state_s = [
            torch.stack(s).reshape(model.term_length, batch_size, -1)
            for s in model.state_s
        ]

        fig, ax = plt.subplots(
            nrows=len(len_state_v) + len(state_s) + 1,
        ...
        for i in range(len(batch_idxs)):
            ...
            if self.task == "nmnist":
                ax[0, i].scatter(
                    input.cpu()
                    .reshape(batch_size, model.time_length, -1)[bidx]
                    .nonzero()[:, 0],
                    input.cpu()
                    .reshape(batch_size, model.time_length, -1)[bidx]
                    .nonzero()[:, 1],
            ...
            for l in range(len(state_v)):
                ...
                ax[sidx, i].scatter(
                    state_s[l][:, bidx, :].cpu().nonzero()[:, 0],
                    state_s[l][:, bidx, :].cpu().nonzero()[:, 1],
                    s=1,
                    c="r",
                )
