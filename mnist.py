...
parser = argparse.ArgumentParser(
    description="ANTLR MNIST Example Arguments", fromfile_prefix_chars="@"
)
parser.add_argument(
    "-t", "--tag", type=str, default="", metavar="S", help="tag for the run"
)
parser.add_argument(
    "-s",
    "--random-seed",
...
parser.add_argument(
    "--eval-model", type=str, metavar="S", help="enable evaluation mode"
)
...
parser.add_argument(
    "--num-workers", type=int, default=1, metavar="N", help="(default: 1)"
)
parser.add_argument(
    "--inf-speed-test", type=int, default=0, metavar="N", help="(default: 0)"
)
...
model_args.add_argument(
    "--grad-clip", type=str, default="1.0", metavar="F", help="(default: 1.0)"
)

model_args.add_argument(
    "--multi-model", type=int, default=0, metavar="N", help="Default=0"
)
model_args.add_argument(
    "--num-models", type=int, default=1, metavar="N", help="Default=1"
)
model_args.add_argument(
    "--init-bias-center", type=int, default=0, metavar="N", help="0"
)
model_args.add_argument(
    "--beta-auto", type=int, default=1, metavar="N", help="1"
)
...
def main():
    ...
    if apargs.eval_model is None:
        config.__dict__.update(
            {key: getattr(apargs, key) for key in vars(apargs)}
        )
    else:
    ...
    data_loaders = mnist_dataset.load_loader(
        config=config,
    ...
    if apargs.eval_model is not None:
        param_path = Path(
            f"./logs/{apargs.eval_model}/m0_best_first_model.pt"
        )
        if cuda_enabled:
        ...
        if logger.resume:
            ...
            try:
                if cuda_enabled:
                    ...
                    trainer.load_model(
                        torch.load(
                            param_path_model, map_location=torch.device("cpu")
                        )
                    )
                    trainer.load_optim(
                        torch.load(
                            param_path_optim, map_location=torch.device("cpu")
                        )
                    )
